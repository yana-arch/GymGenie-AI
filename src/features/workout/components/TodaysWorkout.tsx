...
const TodaysWorkout: React.FC<TodaysWorkoutProps> = ({ onStartWorkout }) => {
  ...
  if (exerciseSets.length === 0) {
    return <div className="p-4 text-center text-gray-500">Loading exercises...</div>;
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      <h2 className="text-2xl font-bold mb-4">Your Workout Sets</h2>
      <div className="relative">
        <div className="bg-gray-50 rounded-lg shadow-md p-4">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold text-gray-800">{toTitleCase(currentExerciseSet.name)}</h3>
          ...
          <div className="grid grid-cols-3 gap-2">
            {currentExerciseSet.exercises.slice(0, 3).map((ex) => (
              <div key={ex.id} className="w-full h-32 bg-gray-200 rounded-md overflow-hidden">
                <img src={ex.media?.gif} alt={ex.name} className="w-full h-full object-contain" />
              </div>
            ))}
          </div>
          <button onClick={onStartWorkout} className="mt-4 w-full bg-blue-500 text-white py-2 rounded-md hover:bg-blue-600 transition-colors">
            Start
          </button>
        </div>
        <button
